...
public class WheelAmPmPicker extends WheelPicker<String> {
    ...
    protected String initDefault() {
        if (DateHelper.getHour(DateHelper.today(), true) >= SingleDateAndTimeConstants.MAX_HOUR_AM_PM) {
            return getContext().getResources().getString(R.string.picker_pm);
        } else {
            return getContext().getResources().getString(R.string.picker_am);
        }
    }
    ...
    protected List<String> generateAdapterValues(){
        return Arrays.asList(
                getContext().getResources().getString(R.string.picker_am),
                getContext().getResources().getString(R.string.picker_pm)
        );
    }
