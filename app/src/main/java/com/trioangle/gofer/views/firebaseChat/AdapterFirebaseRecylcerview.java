...
public class AdapterFirebaseRecylcerview extends RecyclerView.Adapter<AdapterFirebaseRecylcerview.RecyclerViewHolder>  {
    ...
    public void onBindViewHolder(@NonNull RecyclerViewHolder holder, int position) {
        if(chatList.get(position).type != null){
            if(chatList.get(position).type.equals(CommonKeys.FIREBASE_CHAT_TYPE_RIDER)){
    ...
    private void handleOpponentProfilePicture(RecyclerViewHolder holder, int position) {
        try{
            if(position!=0){
                if(chatList.get(position).type != null && chatList.get(position-1).type.equals(CommonKeys.FIREBASE_CHAT_TYPE_RIDER)){
                    Picasso.with(mContext).load(sessionManager.getDriverProfilePic()).error(R.drawable.car).into(holder.opponentProfileImageView);
                }else{
